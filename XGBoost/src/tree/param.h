...
struct XGBOOST_ALIGNAS(16) GradStats {
 ...
 public:
  XGBOOST_DEVICE int32_t GetGrad() const { return sum_grad; }
  XGBOOST_DEVICE int32_t GetHess() const { return sum_hess; }
  ...
  explicit GradStats(const int32_t grad, const int32_t hess)
  ...
  /*! \brief add statistics to the data */
  inline void Add(int32_t grad, int32_t hess) {
    //printf("%d %d\n", sum_grad, sum_hess);
    sum_grad += grad;
    sum_hess += hess;
  }
