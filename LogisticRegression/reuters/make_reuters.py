...
def make_reuters_data():
  ...
  X = CountVectorizer(dtype=np.float64, max_features=1000).fit_transform(data)
  Y = np.asarray(target)
  ...
  np.save('train', X[index]);
  np.save('label', Y[index]);
  np.savetxt('train.out', X[index], delimiter=' ')
  np.savetxt('label.out', Y[index] - 1, delimiter=' ')
